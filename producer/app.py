...
def get_vllm_url():
    """Get vLLM URL for direct streaming requests"""
    vllm_host = os.getenv('VLLM_HOST', 'localhost')
    vllm_port = os.getenv('VLLM_PORT', '8000')
    return f"http://{vllm_host}:{vllm_port}"
...
async def proxy_health():
    """Proxy /health requests via RabbitMQ RPC"""
    try:
        request_data = {
            "endpoint": "/health",
            "method": "GET",
        ...
        logger.info("Proxying GET /health via RPC")
        result = rpc_client.call_rpc(request_data)
        ...
        logger.error(f"Error proxying /health: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.api_route("/v1/completions", methods=["POST"])
async def proxy_v1_completions(request: Request):
    """Proxy /v1/completions requests via RabbitMQ RPC or direct streaming"""
    try:
        body = await request.body()
        headers = dict(request.headers)
        body_str = body.decode() if body else None
        ...
        if is_streaming_request(body_str):
            logger.info("Detected streaming request, using direct proxy")
            return await proxy_streaming_request("/v1/completions", "POST", headers, body_str)
        ...
        logger.info("Proxying POST /v1/completions via RPC")
        result = rpc_client.call_rpc(request_data)
        ...
        logger.error(f"Error proxying /v1/completions: {e}")
        raise HTTPException(status_code=500, detail=str(e))
